...
# Інструкції промовляє **сама плата**, а не цей комп'ютер. Так кожна інструкція заразом є
# перевіркою динаміка: якщо ви її почули, значить зв'язок, синтез мови й вихід звуку справні, і
# перевіряти їх окремим тестом уже не треба. Якщо не почули -- тест іще не почався, а несправність
# уже знайдена.
#
# Текст іде звичайним шляхом `text_in`, тому плата промовляє відповідь моделі. Прохання повторити
# дослівно -- найпростіший спосіб отримати рівно потрібні слова, не додаючи заради тесту нового
# типу повідомлення в протокол.
announce() {
    local text="$1"
    echo "  🔊 (на платі) $text"
...
test_0() {
    header 0 "чи плата взагалі говорить"
    echo "  ЩО РОБИТИ:  нічого — фразу за вас надішле скрипт. Просто слухайте."
    echo "              Мікрофон тут ні до чого: текст іде на сервер напряму,"
    echo "              відповідь повертається голосом. Це перевіряє динамік,"
    echo "              зв'язок і синтез мови окремо від розпізнавання."
    echo "  ЧОГО ЧЕКАТИ: [state] thinking → replying → idle, і голос із динаміка."
    echo
    announce "Перевірка звуку. Якщо ви це чуєте, динамік працює."
    watch_board 0 "чи плата говорить" 30 "Привіт! Скажи щось про себе." "СЛУХАЙТЕ"
    ask_heard 0
    footer "Якщо тут тиша — проблема в динаміку чи синтезі, не у VAD."
}
...
test_1() {
    ...
    echo
    announce "Тест один. Мовчіть десять секунд. Починаємо."
...
test_2() {
    ...
    echo
    announce "Тест два. Говоріть звичайним голосом десять секунд. Починаємо."
...
test_3() {
    ...
    echo
    announce "Тест три. Говоріть тихіше або відсуньтесь далі. Починаємо."
...
test_4() {
    ...
    echo
    announce "Тест чотири. Нічого не натискайте. Скажіть одну фразу і замовкніть."
...
test_5() {
    ...
    echo
    announce "Тест пʼять. Тридцять секунд тиші. Нічого не робіть."
...
test_6() {
    ...
    echo
    announce "Тест шість. Притисніть і тримайте екран, скажіть фразу, відпустіть."
...
usage() {
    ...
    echo
    echo "  ./tools/hw-tests.sh 0    чи плата говорить ...... просто слухати"
