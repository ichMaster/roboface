...
test_5() {
    header 5 "два пальці вимикають мікрофон — і НЕ лоскочуть"
    echo "  Рев'ю знайшло, що жест вимкнення читався ще й як звичайний тап: обличчя"
    echo "  лоскоталось, а серверу повідомлялось, що пристрій погладили. Персонаж радів"
    echo "  з того, що його змусили замовкнути."
    echo
    echo "  Торкніться екрана ДВОМА пальцями одночасно. Потім ще раз, щоб увімкнути назад."
    echo

    announce "Торкніться мене двома пальцями, щоб вимкнути мікрофон."
    watch_hands 5 "два пальці" 30 "тапніть двома пальцями двічі"
    ...
    ask_choice 5 "Чи ЛОСКОТАЛОСЬ обличчя від цього жесту?" \
...
usage() {
    ...
    echo "  5  два пальці = mute, і не лоскоче      ./tools/touch-tests.sh 5"
