...
def rgb565(r: int, g: int, b: int) -> int:
    """The panel's own colour, and the reason the spec told the artist to quantise before judging."""
...
def encode_luminance(image: Image.Image) -> bytes:
    ...
    """
    return bytes(image.convert("RGBA").convert("L").getdata())


def encode_alpha(image: Image.Image) -> bytes:
    return bytes(image.convert("RGBA").split()[3].getdata())
...
def main() -> int:
    ...
    print(f"{len(blob.entries)} assets -> {OUT_BIN.relative_to(ROOT)}  "
          f"({len(blob.data) / 1024:.0f} KB)")
    for name, offset, length, width, height in blob.entries:
