...
#:
#: **16 KB, and the number was walked back from 4 KB by measurement.** 4 KB looked obviously better
#: on the reasoning above and was ten times worse in practice: `ws 37 ms` before the split, `571 ms`
#: after. Ten times as many frames is ten times as much protocol and TCP work for an ESP32's WiFi
#: stack, and it swamped the benefit of shorter reassembly.
#:
#: So this is a ceiling against a pathological frame rather than a target: it bounds the worst case
#: without multiplying the ordinary one. The lesson is recorded because the reasoning that produced
#: 4 KB was sound and the number was still wrong.
MAX_TTS_FRAME_BYTES: Final = 16384
